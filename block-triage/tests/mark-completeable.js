...
runTest("mark-completeable: Mark as Completeable queues the change instead of applying it right away", async () => {
  ...
  assertEqual(
    ...
    JSON.stringify([{ taskId: "501", status: "0" }]),
    `expected exactly one status-set call (back to Created/0) after processing, got: ${JSON.stringify(mrState.statusSetCalls)}`
  );
...
runTest("mark-completeable: appears in Process all pending's total and gets processed by it", async () => {
  ...
  assertEqual(
    JSON.stringify(mrState.statusSetCalls),
    JSON.stringify([{ taskId: "501", status: "0" }]),
    `expected Process all pending to have processed the completeable queue too, got: ${JSON.stringify(mrState.statusSetCalls)}`
  );
